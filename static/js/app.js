    ...
    <mdui-card class="group-card">
      ...
      <details class="review-box"><summary><span class="material-icons expand-icon" aria-hidden="true">expand_more</span><strong>语言文件</strong><span class="muted">${group.files.filter((item) => !item.deleted && item.extension !== 'json').length} 个文件，点击展开</span></summary><div class="file-list">${group.files.map((item, fileIndex) => item.extension === 'json' ? '' : `
        <div class="file-row ${item.deleted ? 'danger' : ''}">
          ...
          ${item.deleted ? '' : `<details class="review-box"><summary><span class="material-icons expand-icon" aria-hidden="true">expand_more</span><strong>源文件</strong><span class="muted">点击展开预览或修改</span></summary><div class="review-editor"><mdui-text-field variant="outlined" label="源文件内容（修改后翻译和导出使用新内容）" rows="12" value="${escapeHtml(item.edited ?? item.text)}" data-archive-source-edit="${groupIndex}:${fileIndex}"></mdui-text-field></div></details>`}
...
function renderArchive() {
  ...
  const exportCard = as.packs
    ? `<mdui-card class="export-card"><div class="config-grid export-grid"><mdui-text-field label="打包下载文件名（自动加 .zip）" value="${escapeHtml(as.exportName)}" id="archive-filename"></mdui-text-field></div><div class="toolbar"><span class="muted">每个资源包保留原有后缀，统一打进一个 zip</span><div class="toolbar-actions"><mdui-button variant="tonal" id="download-each-pack"><span class="material-icons" aria-hidden="true">download</span>逐包下载</mdui-button><mdui-button variant="filled" id="export-archive"><span class="material-icons" aria-hidden="true">folder_zip</span>打包下载全部（${as.packs.length} 个资源包）</mdui-button></div></div></mdui-card>`
...
function generatedList(group, groupIndex) {
  ...
  return `<div class="generated-list"><h3>已翻译文件（${group.generated.length}）</h3>${group.generated.map((gen, genIndex) => `
    <details class="review-box">
      ...
      <div class="review-editor"><mdui-text-field variant="outlined" label="校对译文（完整文件内容）" rows="12" value="${escapeHtml(gen.edited ?? renderLines(gen.parsed, gen.translated))}" data-review-lang="${groupIndex}:${genIndex}"></mdui-text-field></div>
    </details>`).join('')}</div>`;
}
...
function singleResultCard() {
  ...
  return `<mdui-card class="single-card"><div class="generated-list"><h3>已翻译文件（${results.length}）</h3>
    ...
    <details class="review-box">
      ...
      <div class="review-editor"><mdui-text-field variant="outlined" label="校对译文（完整文件内容）" rows="12" value="${escapeHtml(result.edited ?? renderLines(result.parsed, result.translated))}" data-single-review-lang="${index}"></mdui-text-field></div>
    </details>`).join('')}</div>
    <div class="config-grid export-grid"><mdui-text-field label="打包下载文件名（自动加 .zip）" value="${escapeHtml(state.single.exportName)}" id="single-filename"></mdui-text-field></div>
...
function batchResultsCard(item, fileIndex) {
  ...
  return `<div class="generated-list"><h3>已翻译文件（${results.length}）</h3>${results.map((result, index) => `
    <details class="review-box">
      ...
      <div class="review-editor"><mdui-text-field variant="outlined" label="校对译文（完整文件内容）" rows="12" value="${escapeHtml(result.edited ?? renderLines(result.parsed, result.translated))}" data-batch-review-lang="${fileIndex}:${index}"></mdui-text-field></div>
    </details>`).join('')}</div>`;
}
...
function batchExportCard() {
  ...
  return `<mdui-card class="export-card"><div class="config-grid export-grid"><mdui-text-field label="打包下载文件名（自动加 .zip）" value="${escapeHtml(state.singleBatch.exportName)}" id="single-filename"></mdui-text-field></div><div class="toolbar"><span class="muted">每个源文件单独一个文件夹，与译文一起打包</span><mdui-button variant="filled" id="download-all-batch"><span class="material-icons" aria-hidden="true">folder_zip</span>打包下载全部（${translated} 个译文文件）</mdui-button></div></mdui-card>`;
}
...
function renderSingleBatch() {
  ...
  root.innerHTML = `${translateAllCard}${files.map((item, index) => `
    <mdui-card class="single-card">
      ...
      <details class="review-box"><summary><span class="material-icons expand-icon" aria-hidden="true">expand_more</span><strong>源文件</strong><span class="muted">点击展开预览或修改</span></summary><div class="review-editor"><mdui-text-field variant="outlined" label="源文件内容（修改后翻译使用新内容）" rows="12" value="${escapeHtml(item.edited ?? renderLines(item, item.entries.map((entry) => entry.value)))}" data-batch-source-edit="${index}"></mdui-text-field></div></details>
